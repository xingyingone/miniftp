...
*/
	...
	signal(SIGCHLD, handle_sigchld);					//子进程退出，父进程收到SIGCHLD信号
	...
	while (1)
	{
		...
		if (pid == 0)
		{
			...
			sess.ctrl_fd = conn;
			check_limits(&sess);
			signal(SIGCHLD, SIG_IGN);		//防止nobody进程处理ftp服务进程
			begin_session(&sess);
		}
		else
		{
...
void check_limits(session_t *sess)
{
	if (tunable_max_clients > 0 && sess->num_clients > tunable_max_clients)
	{
		...
		exit(EXIT_FAILURE);								//子进程退出后，父进程对子进程进行处理，连接数减一
	}
...
void handle_sigchld(int sig)
{
	...
	{
		--s_children;
		unsigned int *ip = hash_lookup_entry(s_pid_ip_hash, &pid, sizeof(pid));//更改的是父进程的哈希表
		if (ip == NULL)
		{
			continue;
		}
